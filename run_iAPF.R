run_iAPF <- function(model, data, Napf){
  ...
  for(index in 1:2){
    ...
    for(b in 2:length(breaks[[index]])){
      ...
      Z_apf <- vector()
      N[l] = Napf    
      output <- run_psi_APF(model, list(obs[breaks[[index]][(b-1)]:(breaks[[index]][b]-1),], breaks[[index]][(b-1):b], run_iAPF <- function(model, data, Napf){
        breaks <- data$breaks
        psi_index <- data$psi_index
        obs <- data$obs
        Time <- nrow(obs)
        ...
        for(index in 1:2){
          ...
          for(b in 2:length(breaks[[index]])){
            ...
            Z_apf <- vector()
            N[l] = Napf    
            output <- run_psi_APF(model, list(obs[breaks[[index]][(b-1)]:(breaks[[index]][b]-1),], breaks[[index]][(b-1):b], w_apf[nrow(w_apf),], X_apf[nrow(X_apf),,]), N[l], psi_pa = 0, init = TRUE)
            X_apf <- output[[1]]
            w_apf <- output[[2]]
            Z_apf[l] <- output[[3]]
